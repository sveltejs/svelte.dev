...
An app error is one that was thrown via the [`error`](/docs/kit/@sveltejs-kit#error) helper from `@sveltejs/kit`, as in `src/routes/expected/+page.server.js`:
...
Any other error — such as the one in `src/routes/unexpected/+page.server.js` — is treated as unknown:
