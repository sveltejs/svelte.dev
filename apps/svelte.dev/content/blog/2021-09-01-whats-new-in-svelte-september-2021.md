...
- The new [`sequence` helper lets you chain together multiple `handle` calls](https://svelte.dev/docs/kit/modules#sveltejs-kit-hooks)
- A new [`handleError` hook](https://svelte.dev/docs/kit/hooks#handleerror) gives you the option to send data to an error tracking service, or to customise the formatting before printing the error to the console.
