...
- `svelte.config.js` now contains functions for setting certain options (`css`, `runes`, `customElement`) so that there's a single source of truth for everything that needs to interact with Svelte config (**svelte@5.54.0**, [Docs](https://svelte.dev/docs/kit/configuration), [#17951](https://github.com/sveltejs/svelte/pull/17951))
...
- Error boundaries are now allowed to catch errors on the server (**kit@2.54.0**, [Docs](https://svelte.dev/docs/kit/errors), [#15308](https://github.com/sveltejs/kit/pull/15308))
