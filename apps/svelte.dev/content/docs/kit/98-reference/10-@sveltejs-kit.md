...
The path to a module whose default export is a [custom handler](/docs/kit/writing-adapters#Custom-request-handler).
...
The [custom handler](/docs/kit/writing-adapters#Custom-request-handler) an adapter can specify to customise request handling. It is expected to return a function which returns a `Response` or calls `server.respond` with any [platform-specific context](/docs/kit/types#Platform).
...
type SSRHandler = (server: {
	respond: (
		request: Request,
		options?: Pick<RequestOptions, 'platform'>
	) => Promise<Response>;
}) => MaybePromise<
