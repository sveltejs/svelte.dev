...
```dts
type ApplyReroute = (request: Request) => Promise<Request>;
```
...
- <span class="tag">returns</span> true if the `reroute` hook exists, false otherwise
- <span class="tag since">available since</span> v3.0.0
...
[generate a server-side manifest](/docs/kit/@sveltejs-kit#Builder)
