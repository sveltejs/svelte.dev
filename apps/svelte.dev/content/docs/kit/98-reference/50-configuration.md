...
This option is deprecated. Use [subpath imports](https://nodejs.org/api/packages.html#subpath-imports) (see [`#lib`](/docs/kit/$lib)) instead. Projects that want bundler and TypeScript aliases configured in one place can set [`resolve.tsconfigPaths`](https://vite.dev/config/shared-options#resolve-tsconfigpaths) in their Vite config.
