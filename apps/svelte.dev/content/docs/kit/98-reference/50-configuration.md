...
- The server has an opportunity to intercept each navigation before it reaches SvelteKit (for example through a reverse proxy, CDN, or custom server middleware), enabling (for example) A/B testing opaque to SvelteKit. This does not run [`handle`](/docs/kit/hooks#Server-hooks-handle) during route resolution itself.
