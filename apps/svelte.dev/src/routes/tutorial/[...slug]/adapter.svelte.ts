...
import type { state as WCState } from '#lib/tutorial/adapters/webcontainer/index.svelte.ts';
import type { state as RollupState } from '#lib/tutorial/adapters/rollup/index.svelte.ts';
...
export function load_webcontainer(force = false) {
	...
	wc_ready = new Promise(async (fulfil, reject) => {
		try {
			...
			const module = await import('#lib/tutorial/adapters/webcontainer/index.svelte.ts');
...
export function load_rollup(force = false) {
	...
	rollup_ready = new Promise(async (fulfil, reject) => {
		try {
			const module = await import('#lib/tutorial/adapters/rollup/index.svelte.ts');
