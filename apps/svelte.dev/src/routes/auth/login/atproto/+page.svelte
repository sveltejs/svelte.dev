<script lang="ts">
	...
	async function start(actor: string, private_apps: boolean) {
		busy = true;
		message = '';
		try {
			const url = await atproto.authorize_url(actor, { private_apps, can_delete });
			location.assign(url);
		} catch (e) {
	...
	onMount(() => {
		...
		if (!user) message = 'Log in first';
		else if (!user.spaces_supported) message = 'This PDS does not support spaces';
		else return start(user.did, true);
		busy = false;
	});

	function submit(e: SubmitEvent) {
		e.preventDefault();
		const handle = normalize(
			new FormData(e.target as HTMLFormElement).get('handle')?.toString() ?? ''
		);
		if (!handle) return;
		// an account that already enabled private apps keeps that scope across logins
		const existing = atproto.current();
		start(
			handle,
			existing?.handle === handle || existing?.did === handle ? existing.private_apps : false
		);
	}
</script>
